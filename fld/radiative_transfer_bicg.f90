  ...
  do i=1,nb_ind
     ...
     do igroup=1,ngrp
        ...
        if(rt_feedback)then !
         ...
         !   kappaR_bicg(this,igroup) = rosseland_ana(3.870970818875849E-017,10.0000000000000,11.0000000000000,igroup,in_sink(this)) / scale_kappa
         !   write(*,*)'scale_kappa',scale_kappa
         !   write(*,*)'kappaR_bicg',this,igroup,kappaR_bicg(this,igroup),density,temp,tr !Testwrite
        end if
        if(kappaR_bicg(this,igroup)*dx_loc .lt. min_optical_depth) kappaR_bicg(this,igroup)=min_optical_depth/dx_loc
            ! write(*,*)'Minoptdepth',min_optical_depth,dx_loc !!Testwrite
            ! write(*,*)'instead setting kappa to', kappaR_bicg(this,igroup) !Testwrite
     enddo
  end do
#endif
...
!!$             & -unew(liste_ind(i),firstindex_er+igrp))
     ...
     if(rt_protostar_m1 .and. rt_advect .and. nsink >0 .and. Teff_sink(1) > 0.0d0)then !heating term by M1 photons absorbed
        do im1=1,ngroups !!! Loop over M1 photon groups
           protostellar_heating = rtuold(liste_ind(i),iGroups(im1))*scale_Np*group_egy(im1)*eV2erg*planck_ana(rho*scale_d,Told,Teff_sink(1),1,in_sink(liste_ind(i)))*rt_c_cgs*z_ave / (scale_d*scale_v**2)
           end do
     end if
#endif
...
subroutine compute_residual_in_cell(i,vol_loc,residual,mat_residual)
  ...
  do igroup=1,ngrp
     ...
     if(rt_protostar_m1 .and. rt_advect .and. nsink >0 .and. Teff_sink(1) > 0.0d0)then !heating by M1 photons into CvT
       do im1=1,ngroups
           protostellar_heating = rtuold(i,iGroups(im1))*scale_Np*group_egy(im1)*eV2erg*planck_ana(rho*scale_d,Told,Teff_sink(1),1,in_sink(i))*rt_c_cgs*z_ave / (scale_d*scale_v**2)
        end do
     end if
#endif
