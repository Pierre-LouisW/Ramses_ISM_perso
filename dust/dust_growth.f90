...
!  use smol2other
  ...
  real(dp):: e,epsilon_0,sum_dust,vrel,s_dust,vfrag_adim,x0,dx,pi,scale,dx_loc,v_brownian,d,u,v,w,enint,cs,ekin,emag,rossby_disk,sc_number,a,b,c,t_stop,t_growth,A_gain_loss,temp,vd,ud,wd,vrel2
  integer::idust,jdust,irad,Q_coag
  ...
  real(dp), dimension(:), allocatable :: vecnodes,vecweights
  real(dp),dimension (1:ndust+1) :: massgrid
  real(dp),dimension (1:ndust) :: massbins,gij,gijnew
  real(dp):: tstop1,tstop2,tkol,dvl,tl,dv1,dv2,dv3,lambda_J,Reyn,a_gas,vrelB,vrelT,vrelD,number_dust
...
#if LIU>0 
  Q_coag=1 ! Nombre de points pour Gauss quadrature
  allocate(vecnodes(Q_coag))
    ...
    do ind=1,twotondim
      ...
      if (stepinski) then      
         do i=1,ngrid
            ...
            call temperature_eos(d*(1.0d0-sum_dust),enint,temp,ht)
            a_gas = sqrt(f(ind_cell(i),1)**2.+f(ind_cell(i),2)**2.+f(ind_cell(i),3)**2.)
            tl    = cs/a_gas
            !print*, a_gas, d, cs, sum_dust

            do idust=1,ndust
!!$               ud = v_dust(ind_cell(i),idust,1)
!!$               vd = v_dust(ind_cell(i),idust,2)
!!$               wd = v_dust(ind_cell(i),idust,3)
               s_dust=uold(ind_cell(i),firstindex_dustpscal+idust)/uold(ind_cell(i),firstindex_ndust+idust)
               tstop1= sqrt(pi*gamma/8.0d0)*s_dust*d_grain(idust)/d/cs
               vrelT = 1.5d0*sqrt(alpha_disk)*cs*sqrt(tstop1/tl) !Ormel & Cuzzi with alpha turb
               vrelB = sqrt(pi/8.0d0*(mH/scale_m)/(4./3.*pi*d_grain(idust)*s_dust**3.))*cs
               vrel  = sqrt(vrelT**2+vrelB**2.)
               number_dust=uold(ind_cell(i),firstindex_ndust+idust)/(4./3.*pi*s_dust**3.*d_grain(idust))
               t_growth=1./(pi*s_dust**2.*number_dust*vrel)
               A_gain_loss=min(1.0d0,-log10(vrel/(v_frag/scale_v)/log10(5.0d0)))
               s_dust=max(s_dust*(1.0d0+A_gain_loss*dtnew(ilevel)/(3.0d0*t_growth)),l_grain(1))
