...
subroutine dustdifffine1(ind_grid,ncache,ilevel)
  ...
  real(dp)::rho_gas, t_stop,t_stop_floor,dens_floor,d0,r0
     ...
     iskip=ncoarse+(ind_son-1)*ngridmax
        ...
        do i=1,ncache
           if(son(ind_cell(i))==0)then
              ...
              !if(update_eint.and..not.dust_diffusion_test) print *, 'toto', flux_dust(i,i3,j3,k3,ndust+1+ndust_pscal*ndust,idim), idim
         ...
         do i3=i3min,i3max-i0
            do i=1,nb_noneigh
               unew(ind_buffer(i),5)=unew(ind_buffer(i),firstindex_ndust+ndust+1+ndust_pscal*ndust) &
                         &-flux_dust(ind_cell(i),i3,j3,k3,ndust+1+ndust_pscal*ndust,idim)*oneontwotondim
            end do
         end do
      end do
   end do
   ...
   do k3=k3min,k3max+k0
      do j3=j3min,j3max+j0
         do i3=i3min,i3max+i0
            do i=1,nb_noneigh
               unew(ind_buffer(i),5)=unew(ind_buffer(i),firstindex_ndust+ndust+1+ndust_pscal*ndust) &
                    &-flux_dust(ind_cell(i),i3+i0,j3+j0,k3+k0,ndust+1+ndust_pscal*ndust,idim)*oneontwotondim
            end do
         end do
      end do
   end do
