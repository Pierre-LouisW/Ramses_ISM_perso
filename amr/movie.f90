...
subroutine output_frame()
  ...
  integer::npoly
  real(dp)::msol,lumsol,t_year,log_lum
 ...
 do proj_ind=1,LEN(trim(proj_axis))
    ...
    t_year   = yr2sec/scale_t
    if(cosmo) t_year = t_year*aexp**2
...
666                            continue
                            ...
                            if((is_cube .and. (cube_face))                           &
                               ...
                               do kk=1,n_movie_vars
                                  ...
                                  if(movie_vars(kk).eq.i_mv_xhi)then
                                     ok_frame=.true.
                                     uvar = 1d0-uold(ind_cell(i),iIons)/max(uold(ind_cell(i),1),smallr)
                                  else if(movie_vars(kk).eq.i_mv_xhii)then
                                     ok_frame=.true.
                                     uvar = uold(ind_cell(i),iIons)/max(uold(ind_cell(i),1),smallr)
                                  else if(movie_vars(kk).eq.i_mv_xheii)then
                                     ok_frame=.true.
                                     uvar = uold(ind_cell(i),iIons+1)/max(uold(ind_cell(i),1),smallr)
                                  else if(movie_vars(kk).eq.i_mv_xheiii)then
                                     ok_frame=.true.
                                     uvar = uold(ind_cell(i),iIons+2)/max(uold(ind_cell(i),1),smallr)
                                  endif
    ...
    do j=1,npartmax
       ...
       do kk=1,n_movie_vars
          if(movie_vars(kk) .eq. i_mv_dm .and. (tp(j).eq.0d0))then
             ! DM particles
          ...
          else if(star .and. movie_vars(kk) .eq. i_mv_stars .and. (tp(j).ne.0d0))then
             ! Star particles
             data_frame(ii,jj,kk)=data_frame(ii,jj,kk)+mp(j)
          else if(star .and. movie_vars(kk) .eq. i_mv_lum .and. (tp(j).ne.0d0))then
             ...
             if(log10((texp-tp(j))/t_year)<6)then
                log_lum = 3.2d0
             else if(log10((texp-tp(j))/t_year)>9)then
                log_lum = log10((texp-tp(j))/t_year)*(-9.79362D-01)+9.08855D+00
             else
                log_lum = 0d0
                do npoly=1,size(lum_poly)
                   log_lum = log_lum+lum_poly(npoly)*(log10((texp-tp(j))/t_year))**(npoly-1)
                enddo
             endif
