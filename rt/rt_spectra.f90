...
!-------------------------------------------------------------------------
  ...
  la0 = X(1) ; la1 = X(N)
  if(e1.gt.0) la0 = max(la0, 1.d8 * hp * c_cgs / e1 / eV2erg)
  if(e0.gt.0) la1 = min(la1, 1.d8 * hp * c_cgs / e0 / eV2erg)
...
FUNCTION getCrosssection_Hui(lambda, species)
  ...
  !------------------------------------------------------------------------
  use rt_parameters,only: ionEvs, hp
  use constants, only:eV2erg,c_cgs
  ...
  real(kind=8)      :: E, x, y
  !------------------------------------------------------------------------
  E = hp * c_cgs/(lambda*1.d-8) / eV2erg         ! photon energy in ev
...
!-------------------------------------------------------------------------
  use rt_parameters,only: hp, SED_isEgy
  use constants, only:eV2erg,c_cgs
  use spectrum_integrator_module
...
!-------------------------------------------------------------------------
  ...
  if(.not. SED_isEgy) then               !  Photon number per sec per Msun
     ...
     getSEDLuminosity = getSEDLuminosity/eV2erg*Lsun
  endif
...
!-------------------------------------------------------------------------
  ...
  integer :: N,species
  real(dp),parameter :: const=1.d8*hp*c_cgs/eV2erg! energy conversion
!-------------------------------------------------------------------------
  species      = 1                       ! irrelevant but must be included
...
!-------------------------------------------------------------------------
  ...
  use rt_parameters
  use constants, only:M_sun,sec2Gyr,gyr2sec
...
!_________________________________________________________________________
  use amr_parameters,only:dp
  use spectrum_integrator_module
  use rt_parameters,only: hp, nIons, ionEVs, nGroups
  use constants, only:eV2erg,c_cgs
...
!-------------------------------------------------------------------------
  ...
  real(kind=8),parameter :: const2=fourPi*eV2erg
!-------------------------------------------------------------------------
  e0=ionEvs(species)
  getUV_Hrate = &
...
!-------------------------------------------------------------------------
  ...
  integer :: N,species
  real(dp),parameter :: const=1.d8*hp*c_cgs/eV2erg    ! unit conversion
!-------------------------------------------------------------------------
  species      = 1                       ! irrelevant but must be included
  norm         = integrateSpectrum(X, Y, N, e0, e1, species, f1)
  getUVEgy  = const * &
