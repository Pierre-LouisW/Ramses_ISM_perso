module amr_parameters
  ...
  real(dp),dimension(1:MAXOUT)::tout=0.0d0,tout_nml=0.0d0       ! Output times
  ...
  logical::disk_refine=.false. ! Refine on disk parameters
  logical::cen_refine=.false.  ! Refine central column to build boundary accels
  logical::middle_refine=.true. ! Refine the central 2^3 cells on init
