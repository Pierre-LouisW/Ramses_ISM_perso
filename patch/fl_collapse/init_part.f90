subroutine init_part
  ...
  integer::i,igrid,ncache,ngrid,iskip,j
  integer::ind,ix,iy,iz,ilun,icpu
  integer::i1,i2,i3
  integer::i1_min=0,i1_max=0,i2_min=0,i2_max=0,i3_min=0,i3_max=0
  integer::buf_count,indglob
  real(dp)::dx,xx1,xx2,xx3,vv1,vv2,vv3,mm1,partSize
  real(dp)::min_mdm_cpu,min_mdm_all,pi
  ...
  real(kind=8),dimension(1:nvector)::mm,ps
...
#endif
  allocate(ptcl_size(npartmax)) 
  !allocate(ptcl_tracer(npartmax)) 
  xp=0.0; vp=0.0; mp=0.0; levelp=0; idp=0; ptcl_size=0
  ...
  if(nrestart>0)then
     ...
     case ('ascii')
        ...
        if (lagrangian_dust)then !modified read for lagrangian dust implementation by FL
           ...
           do while (.not.eof)
              ...
              if(myid==1)then
                 jpart=0
                 do i=1,nvector
                    read(10,*,end=100)partSize,xx1,xx2,xx3
                    jpart=jpart+1
                    indglob=indglob+1
                    ps(i)=partSize+nparttot ! all langragian dust have sizes>npartmax
                    xx(i,1)=0.25*xx1*boxlen+boxlen/2.0
                    xx(i,2)=0.25*xx2*boxlen+boxlen/2.0
                    xx(i,3)=0.25*xx3*boxlen+boxlen/2.0
                    vv(i,1)=0.0
                    vv(i,2)=0.0
                    vv(i,3)=0.0
                    ii(i  )=indglob
...
100              continue
              ...
              buf_count=nvector*3
#ifndef WITHOUTMPI
              call MPI_BCAST(ps,nvector  ,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,info)
              ...
              if(cc(i)==myid)then
                 ...
                 mp(ipart)    =0.0d0!4.*pi/3.*ps(i)**3
                 levelp(ipart)=levelmin
                 idp(ipart)   =ii(i)
                 ptcl_size(ipart)=ps(i)
#ifndef WITHOUTMPI
              endif
#endif
           enddo
        ...
        filename=TRIM(initfile(levelmin))//'/ic_part_tracer'
           if(myid==1)then
              open(10,file=filename,form='formatted')
              indglob=0
           end if
           ...
           do while (.not.eof)
              ...
              if(myid==1)then
                 jpart=0
                 do i=1,nvector
                    read(10,*,end=101)partSize,xx1,xx2,xx3
                    jpart=jpart+1
                    indglob=indglob+1
                    ps(i)=partSize ! all langragian dust have sizes>npartmax
                    xx(i,1)=0.25*xx1*boxlen+boxlen/2.0
                    xx(i,2)=0.25*xx2*boxlen+boxlen/2.0
                    xx(i,3)=0.25*xx3*boxlen+boxlen/2.0
                    vv(i,1)=0.0
...
101              continue
              ...
              if(cc(i)==myid)then
                 ...
                 do j=1,ndust
                    ...
                    levelp(ipart)=levelmin
                    idp(ipart)   =ii(i)
                    ptcl_size(ipart)=j !j dust specie tracer
              end do
#ifndef WITHOUTMPI
              endif
#endif
     ...
     else !if not lagrangian dust implementation by FL then use original implementation
        ...
        eof=.false.
         ...
         do while (.not.eof)
              ...
              if(myid==1)then
                 jpart=0
                 do i=1,nvector
                    read(10,*,end=102)mm1,xx1,xx2,xx3!,vv1,vv2,vv3,mm1
                    jpart=jpart+1
                    indglob=indglob+1
                    xx(i,1)=0.25*xx1*boxlen+boxlen/2.0
                    xx(i,2)=0.25*xx2*boxlen+boxlen/2.0
...
102              continue
  ...
  if(sink .and. .not. isolate_disk)call init_sink
...
use amr_commons
  ...
  do ilevel=levelmin,nlevelmax
     ...
     do jgrid=1,numbl(myid,ilevel)
        ...
        if(npart1>0)then
           ...
           ! Save next particle  <---- Very important !!!
              ...
              ind_grid_part(ip)=ig
              if(ip==nvector)then
                 call kick1(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
                 ip=0
                 ig=0
              end if
     ...
     ! End loop over grids
     if(ip>0)call kick1(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
...
!!$     do idim=1,ndim
!!$        do j=1,np
!!$           vp(j,idim)=uold(indp(j,ind),idim+1)/max(uold(indp(j,ind),1),smallr)*vol(j,ind) !gas velocity at particle position? 
!!$        end do
!!$     end do
!!$  end do

end subroutine initial_dust_kick
!#########################################################################
!#########################################################################
!#########################################################################
!#########################################################################
subroutine kick1(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  use amr_commons
  use pm_commons
  use poisson_commons
