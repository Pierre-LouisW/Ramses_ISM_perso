...
subroutine flag_formation_sites
  ...
  do j=npeaks,1,-1
     ...
     if (smbh)then
        ...
        !ok=ok.and.Icl_dd(jj)<0. !May not matter at this scale
...
!        endif
#else
        !ok=ok.and.grav_term(jj) + kinetic_support(jj)<0.d0 !May not matter
#endif
    !print*,"relevance",relevance(jj),"n_cell",n_cells(jj),"occupied",occupied(jj),"max_dens",max_dens(jj),"d_sink",d_sink,"contracting",contracting(jj)!,"incl_dd(jj)",Icl_dd(jj),"incl_d",Icl_d(jj),"max_dens",max_dens(jj),"j_dens",dens_jeans(jj),"grav",grav_term(jj),"k_s",kinetic_support(jj)
...
!!$        T2=p/d*scale_T2
  ...
  if(fld) then
     ...
     !print*,"gt",grav_term(1:npeaks),"ts",thermal_support(1:npeaks),"ks",kinetic_support(1:npeaks),"ms",magnetic_support(1:npeaks),"rt",rad_term(1:npeaks)
  endif
#else
