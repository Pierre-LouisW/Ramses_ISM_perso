...
#endif
  integer::ilevel,ivar
  real(dp)::Jmax,Jmax_all
  if(verbose)write(*,*)'Entering clean_envelop'
  !compute maximum angular momentum on the grid
  Jmax=0.0d0
  do ilevel=nlevelmax,1,-1
     if(ilevel>=levelmin)call compute_Jmax(ilevel,Jmax)
  end do
  ! Compute maximum error
#ifndef WITHOUTMPI
  call MPI_ALLREDUCE(Jmax,Jmax_all,1,MPI_DOUBLE_PRECISION,MPI_MAX,MPI_COMM_WORLD,info)
  Jmax=Jmax_all
#endif

  ! Cut the material which has angular momentum < 0.001 Jmax
  do ilevel=nlevelmax,1,-1
     if(ilevel>=levelmin)call isolate_disk_fine(ilevel,Jmax)
...
end subroutine clean_envelop
!################################################################
!################################################################
!################################################################
!################################################################
subroutine isolate_disk_fine(ilevel,Jmax)
  use amr_commons
  use hydro_commons
    use pm_commons
  use cooling_module
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        ! Rescale position from code units to user units and put it in the sink frame
        do idim=1,ndim
           do i=1,ngrid
              xx(i,idim)=((xx(i,idim)-skip_loc(idim))*scale)-xsink(1,idim)
              vv(i,idim)=(uold(ind_cell(i),idim+1)/uold(ind_cell(i),1))-vsink(1,idim)
              ! compute angular momentum in the frame of the sink
              J1 = xx(i,2)*vv(i,3) - xx(i,3)*vv(i,2)
              J2 = xx(i,3)*vv(i,1) - xx(i,1)*vv(i,3)
              J3 = xx(i,1)*vv(i,2) - xx(i,2)*vv(i,1)
              Jnorm = sqrt(J1*J1+J2*J2+J3*J3)
              Jmax=max(Jnorm,Jmax)
           end do
        end do
     end do
...
end subroutine isolate_disk_fine
!################################################################
!################################################################
!################################################################
!################################################################
subroutine compute_jmax(ilevel,Jmax)
  use amr_commons
  use hydro_commons
  use pm_commons
  use cooling_module
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        !do idim=1,ndim
           do i=1,ngrid
              xx(i,1:3)=((xx(i,1:3)-skip_loc(1:3))*scale)-xsink(1,1:3)
              vv(i,1:3)=(uold(ind_cell(i),2:4)/uold(ind_cell(i),1))-vsink(1,1:3)
              ...
              J3 = xx(i,1)*vv(i,2) - xx(i,2)*vv(i,1)
              Jnorm = sqrt(J1*J1+J2*J2+J3*J3)
              if(Jnorm<0.001*Jmax)then
                 etot=uold(ind_cell(i),5)
                 
                 etot=etot- 0.5*(uold(ind_cell(i),2)**2+uold(ind_cell(i),3)**2+uold(ind_cell(i),4)**2)/uold(ind_cell(i),1)
#ifdef SOLVERmhd
                 ...
                 etot=etot-0.125d0*((bx1+bx2)**2+(by1+by2)**2+(bz1+bz2)**2)
#endif
#if NENER>0
                 do irad=1,nener
                    etot=etot-uold(ind_cell(i),inener-1+irad)
                 end do
#endif
                 !compute specific energy
                 etot=etot/uold(ind_cell(i),1)
                 if(energy_fix)ei=uold(ind_cell(i),nvar)/uold(ind_cell(i),1)
                 
           
                 uold(ind_cell(i),1)=uold(ind_cell(i),1)*fac_red
                 uold(ind_cell(i),2:4)=uold(ind_cell(i),2:4)*fac_red
...
#endif
                 uold(ind_cell(i),5)=etot
                 if(energy_fix)ei=uold(ind_cell(i),nvar)*uold(ind_cell(i),1)
                                  
                                  
                 ! update passive scalars
                 do ivar=imetal,lastindex_pscal
                    uold(ind_cell(i),ivar)=uold(ind_cell(i),ivar)*fac_red
                 end do
