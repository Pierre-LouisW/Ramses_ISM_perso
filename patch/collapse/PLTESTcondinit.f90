...
subroutine condinit(x,u,dx,nn)
  ...
  if(bb_test)then
     ...
     !PLW Artificially setting rotation to zero:
     ! Setting rotation matrix to identity
    omega0 = 0.0d0
    rot_M(1,1:3) = (/1.0d0, 0.0d0, 0.0d0/)
    rot_M(2,1:3) = (/0.0d0, 1.0d0, 0.0d0/)
    rot_M(3,1:3) = (/0.0d0, 0.0d0, 1.0d0/)

    rot_invM(1,1:3) = (/1.0d0, 0.0d0, 0.0d0/)
    rot_invM(2,1:3) = (/0.0d0, 1.0d0, 0.0d0/)
    rot_invM(3,1:3) = (/0.0d0, 0.0d0, 1.0d0/)

    rot_tilde(1,1:3) = (/0.0d0, 0.0d0, 0.0d0/)
    rot_tilde(2,1:3) = (/0.0d0, 0.0d0, 0.0d0/)
    rot_tilde(3,1:3) = (/0.0d0, 0.0d0, 0.0d0/)
...
subroutine calc_boxlen
  ...
  implicit none
  !================================================================
  !this routine calculate boxlen !PLW removed previous routine and added tine's from condinit_cloud
  !================================================================
  integer :: i
  real(dp):: d_c,zeta
  ...
  real(dp),save:: first
  real(dp):: mu=2.31d0 ! NOTE - MUST BE THE SAME AS IN units.f90!!
!  real(dp)::myid
