...
! Cosmic rays energy groups
  ...
  !Disk & Streaming parameters
  logical  :: shear_terms=.true.  
  real(dp) :: Omega_shear = 1.0d0
  real(dp) :: q_shear = 1.0d0
  logical  :: shear =.true.
  real(dp) :: rho0 = 1.0d0
  real(dp) :: rhod0= 2.0d0
  real(dp) :: ky_0=6.0d0
  real(dp) :: kz_0=6.0d0
  real(dp) :: Radii0 =1.0d0
  real(dp) :: turb_perc= 0.01d0
  real(dp) :: hover_r=0.05
  real(dp) :: amplitude_pert=0.01d0
#if NDUST>0  
  real(dp),dimension(1:ndust) :: Stokes_number_idust = 0.1d0
#endif  
  ! modif nimhd
  integer:: nxx=1
  integer:: nyy=2
