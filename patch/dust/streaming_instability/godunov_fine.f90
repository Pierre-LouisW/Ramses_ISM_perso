...
subroutine set_uold(ilevel)
  ...
  if(dust_diffusion)then
    ...
    !PH and CC 7/2017
  if(shear)then
     if(shear_terms)call add_shear_terms(ilevel)
  endif
...
!###########################################################
!###########################################################
subroutine add_shear_terms(ilevel)
  use amr_commons
  use hydro_commons
  ...
  integer::ilevel
  !---------------------------------------------------------
  ! This routine adds the Coriolis forces and the centrifugal one
  ! to the velocity
  !---------------------------------------------------------
  integer::i,ind,iskip,nx_loc,ix,iy,iz,idust
  integer::ncache,igrid,ngrid,idim,irad
  integer,dimension(1:3,1:2,1:8)::iii,jjj
  ...
  real(dp)::Omeg_shear,e_kin,d_old,e_prim
  real(dp)::A,B,C,e_mag,Ohmdt,alpha,cs,Temp0,sum_dust
...
!  dx_loc=dx/scale
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        ! Gather cell centre y-positions to get the centrifigal force
        do idim=1,ndim
           do i=1,ngrid
        ...
        do i=1,ngrid
              ...
              e_kin=0.5*d*(u**2+v**2+w**2)
              e_mag=0.5* (0.5*(unew(ind_cell(i),6)+unew(ind_cell(i),nvar+1)))**2.0+(0.5*(unew(ind_cell(i),7)+unew(ind_cell(i),nvar+2)))**2.0+(0.5*(unew(ind_cell(i),8)+unew(ind_cell(i),nvar+3)))**2.0

#if NENER>0
              do irad=1,nener
                 e_mag=e_mag+unew(ind_cell(i),8+irad)
              end do
#endif

              e_prim=unew(ind_cell(i),5) - e_mag - e_kin
              d_old = d
              d=max(unew(ind_cell(i),1),smallr)

              e_prim = e_prim * d / d_old

              unew(ind_cell(i),1) = d


              Ohmdt= Omeg_shear*dtnew(ilevel)

              sum_dust=0.0d0
#if NDUST>0              
              do idust=1,ndust
                 sum_dust=sum_dust+unew(ind_cell(i),firstindex_ndust+idust)/d
              end do
#endif  
              !Todo ADD PRESSURE GRADIENT HERE
              unew(ind_cell(i),2) = d*( u*(1.-Ohmdt**2) + 2.*v*Ohmdt + 2.*q_shear*(xx(i,2)-0.5*boxlen)*Omeg_shear*Ohmdt**2 ) / (1.+Ohmdt**2) 

              unew(ind_cell(i),3) = d*( v*(1.-Ohmdt**2) - 2.*u*Ohmdt + 2.*q_shear*(xx(i,2)-0.5*boxlen)*Omeg_shear*Ohmdt ) / (1.+Ohmdt**2) 

              unew(ind_cell(i),4) = d*w

              unew(ind_cell(i),3) =unew(ind_cell(i),3)-dtnew(ilevel)*(hover_r*omega_shear)**2.*Radii0*2.0d0*d
              
              e_kin = 0.5*(unew(ind_cell(i),2)**2+unew(ind_cell(i),3)**2+unew(ind_cell(i),4)**2) / d 
              sum_dust=0.0d0
#if NDUST>0              
              do idust=1,ndust
                 sum_dust=sum_dust+unew(ind_cell(i),firstindex_ndust+idust)/d
...
#endif              
              !unew(ind_cell(i),5) = (1.0d0-sum_dust)*d*cs**2.0/(gamma-1.0d0) + e_kin + e_mag  ! Isothermal hack
              unew(ind_cell(i),5) = e_prim + e_kin + e_mag  
              !unew(ind_cell(i),5) = d*cs**2.0*(gamma-1.0d0) + e_kin + e_mag  ! Isothermal hack
...
111 format('   Entering add_shear_terms for level ',i2)

end subroutine add_shear_terms
!###########################################################
!###########################################################
!###########################################################
!###########################################################
