     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
                 ...
                 do idust = 1,ndust                 
                    t_stop(idust) =  Stokes_number_idust(idust)/omega_shear
                 ...
                 do idim=1,ndim
                    if(idim==1) then
                       !Todo add effective pressure gradient
                 ...
                 !Streaming term
                 grad_P(2)=grad_P(2)+(hover_r*omega_shear)**2.*Radii0*2.0d0*d*(1.0d0-sum_dust)
                 do idust=1,ndust
...
subroutine ctoprimdust(uin,q,dt,ngrid)
  ...
  use hydro_parameters
  use const
  use radiation_parameters,only:small_er,mu_gas
  use cooling_module      , only : kb,mh
  use units_commons
  implicit none
     ...
     do j = ju1, ju2
        do i = iu1, iu2
           ...
           do l = 1, ngrid
              ...
              !q(l,i,j,k,5)=(1.0d0-sum_dust)*q(l,i,j,k,1)*cs**2.0
