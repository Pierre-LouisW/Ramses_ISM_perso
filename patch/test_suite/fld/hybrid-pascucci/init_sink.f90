subroutine init_sink
  ...
  rsink_star(1:nsinkmax) = rstar*Rsun/scale_l
...
!        read(ilun)ncloud_sink
  ...
  if (nrestart>0)then
     ...
     rsink_star(1) = rstar*Rsun/scale_l
     ...
     rsink_star(1) = rstar*Rsun/scale_l
     Teff_sink(1)=Tstar
!!$  lum_sink(1)=(5.67d-5*(5800.**4))*3.1415/2.0d0*(6.96d10**2)/(scale_d*(scale_l)**2*(scale_v)**3)
     lum_sink(1)=(5.67d-5*(Teff_sink(1)**4))*4.0d0*3.1415d0*(rstar*6.96d10)**2/(scale_d*(scale_l)**2*(scale_v)**3)

!!$     pi=acos(-1.0d0)
!!$     write(*,*) '1',lum_sink(1)
!!$     write(*,*) '2',radiation_source(Teff_sink(1),1)/(scale_d*scale_v**2)*(pi*rsink_star(1)**2*clight/scale_v)
!!$     write(*,*) '3',radiation_source(Teff_sink(1),2)/(scale_d*scale_v**2)*(pi*rsink_star(1)**2*clight/scale_v)
!!$     stop
     !write(*,*) "lum sink, rstartinit, rsinkstar, teff_sink=", lum_sink(1), rstar_init, rsink_star(1),Teff_sink(1)
     !write(*,*)"nsink, xsink(1,1:3), msink(1)", nsink, xsink(1,1:3), msink(1)
