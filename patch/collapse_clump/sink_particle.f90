...
subroutine grow_sink(ilevel,on_creation)
  ...
  msink_new=0d0; xsink_new=0.d0; vsink_new=0d0; delta_mass_new=0d0; lsink_new=0d0; M_jet_new=0d0; M_for_jets=0.d0; vol_tot_for_jets=0.d0
    ...
    if(nsink>0)then
      ...
      do isink = 1 , nsinkmax 
             if(M_for_jets_all(isink)>0.0 .and. vol_tot_for_jets_all(isink) .le. 0.0)then
                ...
                write(*,*) 'isink ',isink, ' M_for_jets_all ', M_for_jets_all(isink) , ' vol_for_jets_all ', vol_tot_for_jets_all(isink)
                write(*,*)'================================================================'
...
!                   if(msink(isink)>0.15*Msun/(scale_d*scale_l**3))then
                   if(msink(isink)>0.07*Msun/(scale_d*scale_l**3))then
...
!           if((star_mass*scale_m/Msun) .gt. 1.d-2)then! .and. (t-tsink(isink))*(scale_t/year) .gt. larson_lifetime)then 
          if(star_mass*scale_m/Msun .gt. 7.d-2)then
...
subroutine print_sink_properties(dMEDoverdt,rho_inf,r2)
  ...
  if (.not. smbh)then
     if(myid==1.and.nsink>0.and. mod(nstep_coarse,ncontrol)==0)then
        ...
        write(*,'(" ================================================================================================================================================================================ ")')
        write(*,'("  Id     M[Msol]      x           y           z        vx       vy       vz      acc_rate[Msol/y] acc_lum[Lsol]  age[y]  int_lum[Lsol]   Teff[K]   Mjet[Msol]        ")')
        write(*,'(" ================================================================================================================================================================================ ")')

        do i=nsink,1,-1
           isink=idsink_sort(i)
           l_abs=(lsink(isink,1)**2+lsink(isink,2)**2+lsink(isink,3)**2)**0.5+1.d10*tiny(0.d0)
           write(*,'(I5,13(2X,E12.5))')&
                ...
                int_lum(isink)*scale_d*scale_l**3*scale_v**2/scale_t/Lsun,Teff_sink(isink),M_jet(isink)*scale_m/Msun
        end do
        write(*,'(" ============================================================================================================================================================================= ")')
     endif
  endif
