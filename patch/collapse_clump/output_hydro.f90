...
#if NDUST>0   
     do ivar=1,npscal-4
        if(firstindex_pscal+ivar-1<firstindex_ndust) then
        ...
        else if((firstindex_pscal+ivar-1.ge.firstindex_ndust).and.(firstindex_pscal+ivar-1<firstindex_dustpscal)) then
        ...
        else if((firstindex_pscal+ivar-1.ge.firstindex_dustpscal).and.(firstindex_pscal+ivar-1<firstindex_dustpscal+ndust_pscal)) then
           idust= firstindex_pscal+ivar-firstindex_dustpscal
           if(idust<10)write(ilun,'("variable #",I2,": dust_size_",I1)')firstindex_pscal+3+ivar,idust
           if(idust.ge.10)write(ilun,'("variable #",I2,": dust_size_",I2)')firstindex_pscal+3+ivar,idust
        else
           if(ivar-ndust-ndust_pscal<10)write(ilun,'("variable #",I2,": passive_scalar_",I1)')firstindex_pscal+3+ivar,ivar-ndust-ndust_pscal
           if(ivar-ndust-ndust_pscal.ge.10)write(ilun,'("variable #",I2,": passive_scalar_",I2)')firstindex_pscal+3+ivar,ivar-ndust-ndust_pscal
        endif
     enddo
#else
     do ivar=1,npscal-4
           ...
           do ind=1,twotondim
              ...
              if(write_conservative) then
                 ...
                 do ivar=1,npscal-4 ! Write passive scalars if any
                    do i=1,ncache
                       if(firstindex_pscal+ivar==firstindex_dustpscal+1)then ! /!\ only one dust pscal is allowed for now
                          xdp(i)=uold(ind_grid(i)+iskip,firstindex_pscal+ivar)/uold(ind_grid(i)+iskip,firstindex_ndust+1)
                       else  
