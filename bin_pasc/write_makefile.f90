subroutine output_makefile(filename)
  ...
  write(ilun,format)""
  write(ilun,format)"NDIM = 2"
  write(ilun,format)"SOLVER = mhd"
  write(ilun,format)"NIMHD=0"
  write(ilun,format)"HALL=0"
  write(ilun,format)""
  write(ilun,format)"NCR = 0"
  write(ilun,format)"NENT =0+$(NCR)"
  write(ilun,format)""
  write(ilun,format)"USE_FLD=1"
  write(ilun,format)"USE_M_1=0"
  write(ilun,format)"NGRP=1"
  write(ilun,format)""
  write(ilun,format)"USE_TURB=0"
  write(ilun,format)""
  write(ilun,format)"NEXTINCT = 0"
  write(ilun,format)"NPSCAL = 0"
  write(ilun,format)"NDUST  = 0"
  write(ilun,format)"NDUSTPSCAL=0"
  write(ilun,format)"GROWTH = 0"
  write(ilun,format)"NENER = $(NENT)+$(NGRP)"
  write(ilun,format)""
  write(ilun,format)"#for RT"
  write(ilun,format)"NGROUPS =1"
  write(ilun,format)"NIONS =3"
  write(ilun,format)""
  write(ilun,format)"PATCH = ../patch/irradiation/test_pascucci"
  write(ilun,format)""
  ...
  write(ilun,format)"   NPSCAL := $(NPSCAL)+$(NIONS)+$(NDUST)+$(NDUST)*$(NDUSTPSCAL)+4 # for internal energy + current"
  write(ilun,format)"else"
  write(ilun,format)"   NPSCAL := $(NPSCAL)+$(NIONS)+$(NDUST)+$(NDUST)*$(NDUSTPSCAL)+1 # for internal energy"
  write(ilun,format)"   HALL=0"
  write(ilun,format)"endif"
  write(ilun,format)"NVAR=8+$(NENER)+$(NEXTINCT)+$(NPSCAL)"
  write(ilun,format)"ifeq ($(USE_M_1),1)"
  write(ilun,format)"   NVAR=$(NVAR)+$(NDIM)*$(NGRP)"
  write(ilun,format)"endif"
  write(ilun,format)""
  write(ilun,format)""
  ...
  write(ilun,format)"## 9+nener+nextinct+nions+1:9+nener+nextinct+nions+ndust : dust species "
  ...
  write(ilun,format)""
  write(ilun,format)"#then do not forget that within the code the magnetic variable are duplicated and stored"
  ...
  write(ilun,format)"          -DSOLVER$(SOLVER) -DNGRP=$(NGRP) -DNPSCAL=$(NPSCAL) -DNIMHD=$(NIMHD) -DHALL=$(HALL)\"
  write(ilun,format)"	  -DNEXTINCT=$(NEXTINCT) -DNCR=$(NCR) -DUSE_TURB=$(USE_TURB) -DNDUST=$(NDUST) -DNDUSTPSCAL=$(NDUSTPSCAL) -DGROWTH=$(GROWTH) #-DQUADHILBERT=$(QUADHILBERT)"
  write(ilun,format)"ifneq ($(NIONS)$(NGROUPS),00)"
  ...
  write(ilun,format)"#   FFLAGS +=  -fallow-argument-mismatch #required for gcc10 "
  ...
  write(ilun,format)"endif"
  write(ilun,format)""
  write(ilun,format)"# Smol"
  write(ilun,format)"ifeq ($(GROWTH),1)"
  write(ilun,format)"LIBS +=-L$(BINDIR) -lsmol_mpif90"
  write(ilun,format)"endif"
  write(ilun,format)"#############################################################################"
  write(ilun,format)"# Sources directories are searched in this exact order"
  write(ilun,format)"BINDIR = ."
  write(ilun,format)"VPATH = $(shell [ -z $(PATCH) ] || find $(PATCH) -type d):../$(SOLVER):../dust:../dust/growth:../turb:../cr:../fld:../nimhd:../aton:../rt:../hydro:../pm:../poisson:../amr"
  write(ilun,format)"#############################################################################"
  write(ilun,format)"# All objects"
  write(ilun,format)"MODOBJ = mpi_mod.o amr_parameters.o amr_commons.o random.o pm_parameters.o \"
  write(ilun,format)"         pm_commons.o poisson_parameters.o constants.o"
  write(ilun,format)"ifeq ($(GRACKLE),1)"
  ...
  write(ilun,format)"         clfind_commons.o gadgetreadfile.o write_makefile.o write_gitinfo.o write_patch.o "
  write(ilun,format)""
  write(ilun,format)"MODOBJ += radiation_parameters.o rt_parameters.o rt_hydro_commons.o coolrates_module.o \"
  write(ilun,format)"	  rt_spectra.o  rt_metal_cooling_module.o rt_cooling_module.o rt_flux_module.o \"
  write(ilun,format)"	  table_1d_module.o singlestar_module.o feedback_module.o cloud_module.o"
  write(ilun,format)"ifeq ($(NIMHD),1)"
  ...
  write(ilun,format)"        flag_formation_sites.o init_sink.o output_sink.o stellar_particle.o init_stellar.o output_stellar.o \"
  write(ilun,format)"	sink_RT_feedback.o output_clumps.o"
  ...
  write(ilun,format)"           radiative_transfer_bicg.o radiation_boundary.o cooling_module_frig.o \"
  write(ilun,format)"	   crdiffusion.o crdiff_split.o crdiffusion_fine.o	"
  write(ilun,format)"# RT objects"
  write(ilun,format)"RTOBJ = rt_init_hydro.o rt_init_xion.o rt_init.o rt_init_flow_fine.o rt_output_hydro.o rt_godunov_fine.o \"
  write(ilun,format)"        rt_interpol_hydro.o rt_godunov_utils.o rt_condinit.o rt_hydro_flag.o rt_hydro_boundary.o rt_boundana.o \"
  write(ilun,format)"        rt_read_hydro_params.o rt_units.o"
  write(ilun,format)"#Dust objects"
  write(ilun,format)"ifneq ($(NDUST),0)"
  write(ilun,format)"DUSTOBJ = dust_fine.o dust_growth.o umuscl_dust.o dust_velocity.o init_dust.o interpol_dust.o"
  write(ilun,format)"ifeq ($(GROWTH),1)"
  write(ilun,format)"DUSTOBJ += precision.o phy_cst.o smol2other.o"
  write(ilun,format)"endif"
  write(ilun,format)"else"
  write(ilun,format)"DUSTOBJ ="
  write(ilun,format)"endif"
  write(ilun,format)""
  write(ilun,format)"ifeq ($(NIMHD),1)"
  ...
  write(ilun,format)""
  write(ilun,format)"# All objects"
  write(ilun,format)"AMRLIB = $(AMROBJ) $(HYDROOBJ) $(PMOBJ) $(POISSONOBJ) $(RTOBJ) $(DUSTOBJ) $(TURBOBJ)"
  write(ilun,format)""
  ...
  write(ilun,format)"	rm -f *.o *.$(MOD) *__genmod.f90"
  write(ilun,format)"#############################################################################"
  write(ilun,format)""
