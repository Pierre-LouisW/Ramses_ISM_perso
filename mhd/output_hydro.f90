...
#else
  write(ilun,'("nvar        =",I11)')nvar+4+ndust*ndim !PLW +3 added for frad 
#endif
  ivar=1
...
#endif
   !outputting rad force (adnan)
   ! write(ilun,'("variable #",I2,": rad_force_x")')nvar + ndust*ndim + ngroups*(1+ndim) +1
   ! write(ilun,'("variable #",I2,": rad_force_y")')nvar + ndust*ndim + ngroups*(1+ndim) +2
   ! write(ilun,'("variable #",I2,": rad_force_z")')nvar + ndust*ndim + ngroups*(1+ndim) +3
  close(ilun)
...
!#############################################################
!#############################################################
subroutine file_descriptor_hydro_main_ramses(filename)
  use amr_commons
  use hydro_commons
  use rt_hydro_commons
  use rt_parameters
  ...
  ! Write run parameters
  write(ilun, '("# version: ", i2)') 1    
  write(ilun, '("# ", a, ", ", a, ", ", a)') 'ivar', 'variable_name', 'variable_type'
  ivar=1
  write(ilun,'(I2," density, d")')ivar
  if(write_conservative) then
     ivar=2
     write(ilun,'(I2," momentum_x, d")')ivar
     ivar=3
     write(ilun,'(I2," momentum_y, d")')ivar
     ivar=4
     write(ilun,'(I2," momentum_z, d")')ivar
  else
     ivar=2
     write(ilun,'(I2," velocity_x, d")')ivar
     ivar=3
     write(ilun,'(I2," velocity_y, d")')ivar
     ivar=4
     write(ilun,'(I2," velocity_z, d")')ivar
  endif
  ivar=5
  write(ilun,'(I2," B_left_x, d")')ivar
  ivar=6
  write(ilun,'(I2," B_left_y, d")')ivar
  ivar=7
  write(ilun,'(I2," B_left_z, d")')ivar
  ivar=8
  write(ilun,'(I2," B_right_x, d")')ivar
  ivar=9
  write(ilun,'(I2," B_right_y, d")')ivar
  ivar=10
  write(ilun,'(I2," B_right_z, d")')ivar
#if NENER>NGRP
  if(write_conservative) then
#if NCR>0  
     ! CR energies
     do ivar=1,ncr
        write(ilun,'(I2," cosmic_rays_energy_",I1,", d")')10+ivar,ivar
     end do
#endif
     ! Non-thermal energies
     do ivar=1+ncr,nent
        write(ilun,'(I2," non_thermal_energy_",I1,", d")')10+ivar,ivar
     end do
  else
#if NCR>0
     ! CR pressures
     do ivar=1,ncr
        write(ilun,'(I2," cosmic_rays_pressure_",I1,", d")')10+ivar,ivar
     end do
#endif
     ! Non-thermal pressures
     do ivar=1+ncr,nent
        write(ilun,'(I2," non_thermal_pressure_",I1,", d")')10+ivar,ivar
     end do
  end if
#endif
  if(write_conservative) then
     ivar=11+nent
     write(ilun,'(I2," total_energy, d")')ivar
  else
     ivar=11+nent
     write(ilun,'(I2," thermal_pressure, d")')ivar
  endif
#if NGRP>0
  ! Radiative energies
  do ivar=1,ngrp
     write(ilun,'(I2," radiative_energy_",I1,", d")')firstindex_er+3+ivar,ivar
  end do
#endif
#if USE_M_1==1
  ! Radiative fluxes
  do ivar=1,ngrp
     write(ilun,'(I2," radiative_flux_x_",I1,", d")')firstindex_fr+3       +ivar,ivar
  end do
if(ndim>1) then
  do ivar=1,ngrp
     write(ilun,'(I2," radiative_flux_y_",I1,", d")')firstindex_fr+3+  ngrp+ivar,ivar
  end do
endif
if(ndim>2) then
  do ivar=1,ngrp
     write(ilun,'(I2," radiative_flux_z_",I1,", d")')firstindex_fr+3+2*ngrp+ivar,ivar
  end do
endif
#endif
#if NEXTINCT>0
  ! Extinction
  do ivar=1,nextinct
     write(ilun,'(I2," extinction_",I1,", d")')firstindex_extinct+3+ivar,ivar
  end do
#endif
#if NPSCAL>0
#if NIMHD==1
  ...
  if(write_conservative) then
     ...
     do ivar=1,npscal-4
        if(firstindex_pscal+ivar.le.firstindex_ndust .or. firstindex_pscal+ivar > firstindex_ndust+ndust ) then
           if(ivar<10)write(ilun,'(I2," passive_scalar_cons_",I1,", d")')firstindex_pscal+3+ivar,ivar
           if(ivar.ge.10)write(ilun,'(I2," passive_scalar_cons_",I2,", d")')firstindex_pscal+3+ivar,ivar
        else if(firstindex_pscal+ivar>firstindex_ndust .and. firstindex_pscal+ivar .le. firstindex_ndust+ndust) then
           idust=firstindex_pscal+ivar-firstindex_ndust
           if(idust<10)write(ilun,'(I2," density_dust_",I1,", d")')firstindex_pscal+3+ivar,idust
           if(idust.ge.10)write(ilun,'(I2," density_dust_",I2,", d")')firstindex_pscal+3+ivar,idust
        endif
     end do
#else
     do ivar=1,npscal-4
        write(ilun,'(I2," passive_scalar_cons_",I1,", d")')firstindex_pscal+3+ivar,ivar
     end do
#endif       
  else
#if NDUST>0
     do ivar=1,npscal-4
        if(firstindex_pscal+ivar .le. firstindex_ndust .or. firstindex_pscal+ivar > firstindex_ndust+ndust ) then
           if(firstindex_pscal+ivar .le. firstindex_ndust) then
              if(ivar<10)write(ilun,'(I2," passive_scalar_",I1,", d")')firstindex_pscal+3+ivar,ivar
              if(ivar.ge.10)write(ilun,'(I2," passive_scalar_",I2,", d")')firstindex_pscal+3+ivar,ivar
           else
              if(ivar-ndust<10)write(ilun,'(I2," passive_scalar_",I1,", d")')firstindex_pscal+3+ivar,ivar-ndust
              if(ivar-ndust.ge.10)write(ilun,'(I2," passive_scalar_",I2,", d")')firstindex_pscal+3+ivar,ivar-ndust
           endif
        else if(firstindex_pscal+ivar.ge. firstindex_ndust .and. firstindex_pscal+ivar .le. firstindex_ndust+ndust) then
           idust=firstindex_pscal+ivar-firstindex_ndust
           if(idust<10) write(ilun,'(I2," dust_ratio_",I1,", d")')firstindex_pscal+3+ivar,idust
           if(idust.ge.10) write(ilun,'(I2," dust_ratio_",I2,", d")')firstindex_pscal+3+ivar,idust
        endif
     end do
#else
     do ivar=1,npscal-4
        write(ilun,'(I2," passive_scalar_",I1,", d")')firstindex_pscal+3+ivar,ivar
     end do
#endif 
  endif
  ivar=npscal-3
  write(ilun,'(I2," current_x, d")')firstindex_pscal+3+ivar
  ivar=npscal-2
  write(ilun,'(I2," current_y, d")')firstindex_pscal+3+ivar
  ivar=npscal-1
  write(ilun,'(I2," current_z, d")')firstindex_pscal+3+ivar
#else
  ! Passive scalars excluding internal energy
  if(write_conservative) then
#if NDUST>0
     ...
     do ivar=1,npscal-1
        if(firstindex_pscal+ivar .le. firstindex_ndust .or. firstindex_pscal+ivar > firstindex_ndust+ndust ) then
           if(ivar-npscal+ndust<10)write(ilun,'(I2," passive_scalar_",I1,", d")')firstindex_pscal+3+ivar,ivar-npscal+ndust
           if(ivar-npscal+ndust.ge.10)write(ilun,'(I2," passive_scalar_",I2,", d")')firstindex_pscal+3+ivar,ivar-npscal+ndust
        else if(firstindex_pscal+ivar.ge. firstindex_ndust .and. firstindex_pscal+ivar .le. firstindex_ndust+ndust) then
           idust=firstindex_pscal+ivar-firstindex_ndust
...
#endif
  ivar=npscal
  write(ilun,'(I2," internal_energy, d")')firstindex_pscal+3+ivar
#endif
  ! Temperature
  ivar=firstindex_pscal+3+npscal+1
  write(ilun,'(I2," temperature, d")')ivar
#if NDUST>0
  do idust=1,ndust
     if(idust<10)write(ilun,'(I2," velocity_drift_",I1,"_x, d")')ivar+idust,idust
     if(idust.ge.10)write(ilun,'(I2," velocity_drift_",I2,"_x, d")')ivar+idust,idust

#if NDIM>1
     if(idust<10)write(ilun,'(I2," velocity_drift_",I1,"_y, d")')ivar+idust+1,idust
     if(idust.ge.10)write(ilun,'(I2," velocity_drift_",I2,"_y, d")')ivar+idust+1,idust

#endif
#if NDIM>2
     if(idust<10)write(ilun,'(I2," velocity_drift_",I1,"_z, d")')ivar+idust+2,idust
     if(idust.ge.10)write(ilun,'(I2," velocity_drift_",I2,"_z, d")')ivar+idust+2,idust
  ...
  if(output_rtvar_in_hydro)then
     write(*,*) "TOTO2 !PLW"
     do ivar=1,nGroups
        write(ilun,'(I2," photon_number_flux_",I1,", d")')nvar+4+ndust*ndim+ivar,ivar
        write(ilun,'(I2," photon_number_xflux_",I1,", d")')nvar+4+ndust*ndim+ivar+1,ivar
        write(ilun,'(I2," photon_number_yflux_",I1,", d")')nvar+4+ndust*ndim+ivar+2,ivar
        write(ilun,'(I2," photon_number_zflux_",I1,", d")')nvar+4+ndust*ndim+ivar+3,ivar
     end do
  end if
#endif
...
end subroutine file_descriptor_hydro_main_ramses
...
subroutine backup_hydro(filename)
  ...
  use rt_parameters
  use mpi_mod
  use radiation_parameters  !,only:frad !PLW
  implicit none
#ifndef WITHOUTMPI
