...
subroutine virtual_tree_fine(ilevel)
  ...
  if(lagrangian_dust)particle_data_width=particle_data_width+1
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  if(lagrangian_dust)then
     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=ptcl_size(ind_part(i))
     end do
     current_property = current_property+1
  end if
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  if(lagrangian_dust)then
     do i=1,np
        ptcl_size(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property)
     end do
     current_property = current_property+1
  end if
